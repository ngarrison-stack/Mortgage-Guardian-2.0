...
public final class DocumentProcessor: ObservableObject {
    ...
    /// Process document with integrated tiered error detection
    public func processDocumentWithTieredAnalysis(
        from imageData: Data,
        fileName: String,
        bankTransactions: [Transaction] = [],
        loanDetails: LoanDetails? = nil,
        errorDetectionConfig: MortgageErrorDetectionService.DetectionConfiguration = .default,
        configuration: OCRConfiguration = .default
        ...
        do {
            updateProgress(.initialization, percentComplete: 0, message: "Starting document processing with tiered error detection")

            // Step 1: Standard document processing (0-40%)
            updateProgress(.preprocessing, percentComplete: 5, message: "Validating and preprocessing document")
            try await validateDocument(data: imageData, fileName: fileName)
            let preprocessedImage = try await preprocessImage(from: imageData)

            updateProgress(.ocrProcessing, percentComplete: 20, message: "Performing OCR text extraction")
            let ocrResult = try await performOCR(on: preprocessedImage, configuration: configuration)

            updateProgress(.dataExtraction, percentComplete: 35, message: "Extracting structured mortgage data")
            let extractedData = try await extractMortgageData(from: ocrResult)
            let documentType = detectDocumentType(from: ocrResult.text)
            ...
            updateProgress(.analysis, percentComplete: 40, message: "Document processing complete, starting tiered error detection")

            // Step 2: Tiered Error Detection Analysis (40-95%)
            let errorDetectionService = MortgageErrorDetectionService(configuration: errorDetectionConfig)
            let analysisResult = try await errorDetectionService.analyzeDocument(
                extractedData: extractedData,
            ...
            updateProgress(.resultValidation, percentComplete: 95, message: "Finalizing analysis results")

            // Step 3: Create comprehensive result
            let finalDocument = MortgageDocument(
                fileName: document.fileName,
                documentType: document.documentType,
                uploadDate: document.uploadDate,
                originalText: document.originalText,
                extractedData: document.extractedData,
                analysisResults: analysisResult.detectedErrors.map { error in
                    // Convert DetectedError to AuditResult for compatibility
                    AuditResult(
                        issueType: mapErrorCategoryToIssueType(error.category),
                        severity: mapErrorSeverityToAuditSeverity(error.severity),
                        title: error.title,
                        description: error.description,
                        detailedExplanation: error.description,
                        suggestedAction: error.suggestedAction,
                        affectedAmount: error.financialImpact,
                        detectionMethod: mapDetectionTierToMethod(error.detectionTier),
                        confidence: error.confidence,
                        evidenceText: error.evidence.map { $0.description }.joined(separator: "; "),
                        calculationDetails: nil,
                        createdDate: error.detectionTimestamp
                    )
                },
                isAnalyzed: true
            )

            updateProgress(.completion, percentComplete: 100, message: "Tiered analysis complete")

            let result = DocumentAnalysisResult(
                document: finalDocument,
                analysisResult: analysisResult,
                processingMetrics: ProcessingMetrics(
                    ocrConfidence: Double(ocrResult.confidence),
                    extractionConfidence: calculateExtractionConfidence(extractedData),
                    processingTime: Date().timeIntervalSinceReferenceDate - startTime,
                    documentComplexity: calculateDocumentComplexity(extractedData),
                    errorDetectionCoverage: analysisResult.analysisMetrics.coveragePercentage
                )
            )

            logger.info("Document processed with tiered analysis: \(fileName), \(analysisResult.detectedErrors.count) errors found")
            return result

        } catch {
            logger.error("Document processing with tiered analysis failed: \(error.localizedDescription)")
            throw error
        }
    }
    ...
    /// Process batch with tiered error detection
    public func processBatchWithTieredAnalysis(
        documents: [(data: Data, fileName: String, bankTransactions: [Transaction], loanDetails: LoanDetails?)],
        errorDetectionConfig: MortgageErrorDetectionService.DetectionConfiguration = .default,
        configuration: OCRConfiguration = .default
    ) async throws -> [DocumentAnalysisResult] {

        guard !isProcessing else {
            throw ProcessingError.ocrProcessingFailed(NSError(domain: "DocumentProcessor", code: -1, userInfo: [NSLocalizedDescriptionKey: "Batch processing already in progress"]))
        }

        isProcessing = true
        defer { isProcessing = false }

        var results: [DocumentAnalysisResult] = []
        let totalBudget = errorDetectionConfig.costBudgetPerDocument * Double(documents.count)
        var remainingBudget = totalBudget

        for (index, documentInfo) in documents.enumerated() {
            updateProgress(.ocrProcessing, percentComplete: Double(index) / Double(documents.count) * 100, message: "Processing document \(index + 1) of \(documents.count) with tiered analysis")

            do {
                // Adjust budget per document based on remaining budget
                let adjustedBudget = min(errorDetectionConfig.costBudgetPerDocument, remainingBudget / Double(documents.count - index))

                var adjustedConfig = errorDetectionConfig
                adjustedConfig = MortgageErrorDetectionService.DetectionConfiguration(
                    enableRuleBasedDetection: adjustedConfig.enableRuleBasedDetection,
                    enableAIAnalysis: adjustedConfig.enableAIAnalysis && adjustedBudget > 0.05, // Disable AI if budget too low
                    aiAnalysisThreshold: adjustedConfig.aiAnalysisThreshold,
                    maxConcurrentAnalyses: adjustedConfig.maxConcurrentAnalyses,
                    ruleBasedTimeout: adjustedConfig.ruleBasedTimeout,
                    aiAnalysisTimeout: adjustedConfig.aiAnalysisTimeout,
                    costBudgetPerDocument: adjustedBudget
                )

                let result = try await processDocumentWithTieredAnalysis(
                    from: documentInfo.data,
                    fileName: documentInfo.fileName,
                    bankTransactions: documentInfo.bankTransactions,
                    loanDetails: documentInfo.loanDetails,
                    errorDetectionConfig: adjustedConfig,
                    configuration: configuration
                )

                results.append(result)
                remainingBudget -= result.analysisResult.costBreakdown.totalCost

                logger.info("Batch item \(index + 1) complete: \(result.analysisResult.detectedErrors.count) errors, cost: $\(String(format: "%.4f", result.analysisResult.costBreakdown.totalCost))")

            } catch {
                logger.error("Failed to process document \(documentInfo.fileName): \(error.localizedDescription)")
                // Continue with next document instead of failing entire batch
            }
        }

        updateProgress(.completion, percentComplete: 100, message: "Batch processing with tiered analysis complete")

        let totalCost = results.reduce(0) { $0 + $1.analysisResult.costBreakdown.totalCost }
        let totalErrors = results.reduce(0) { $0 + $1.analysisResult.detectedErrors.count }

        logger.info("Batch analysis complete: \(results.count) documents, \(totalErrors) total errors, total cost: $\(String(format: "%.4f", totalCost))")
...
private extension DateFormatter {
    ...
    // MARK: - Tiered Error Detection Integration Helpers

    /// Calculate extraction confidence based on data completeness and consistency
    private func calculateExtractionConfidence(_ extractedData: ExtractedData) -> Double {
        var confidence: Double = 1.0

        // Reduce confidence for missing critical fields
        if extractedData.loanNumber == nil { confidence -= 0.15 }
        if extractedData.principalBalance == nil { confidence -= 0.15 }
        if extractedData.interestRate == nil { confidence -= 0.10 }
        if extractedData.monthlyPayment == nil { confidence -= 0.10 }

        // Reduce confidence for incomplete payment records
        let incompletePayments = extractedData.paymentHistory.filter { payment in
            payment.principalApplied == nil || payment.interestApplied == nil
        }.count

        if incompletePayments > 0 {
            confidence -= Double(incompletePayments) / Double(extractedData.paymentHistory.count) * 0.3
        }

        return max(confidence, 0.0)
    }

    /// Calculate document complexity for processing metrics
    private func calculateDocumentComplexity(_ extractedData: ExtractedData) -> Double {
        var complexity: Double = 0.0

        // Base complexity from data volume
        complexity += Double(extractedData.paymentHistory.count) * 0.01
        complexity += Double(extractedData.escrowActivity.count) * 0.02
        complexity += Double(extractedData.fees.count) * 0.03

        // Add complexity for missing data (makes analysis harder)
        let missingFields = [
            extractedData.loanNumber == nil,
            extractedData.principalBalance == nil,
            extractedData.interestRate == nil,
            extractedData.monthlyPayment == nil
        ].filter { $0 }.count

        complexity += Double(missingFields) * 0.05

        return min(complexity, 1.0)
    }

    /// Map error categories to audit result issue types
    private func mapErrorCategoryToIssueType(_ category: MortgageErrorDetectionService.ErrorCategory) -> AuditResult.IssueType {
        switch category {
        case .paymentAllocation, .paymentCalculation:
            return .misappliedPayment
        case .lateFeeMiscalculation:
            return .latePaymentError
        case .interestCalculation, .interestRateApplication:
            return .incorrectInterest
        case .escrowCalculation, .escrowShortage, .escrowOverage:
            return .escrowError
        case .unauthorizedFees, .incorrectFeeAmount:
            return .unauthorizedFee
        case .missingData:
            return .missingPayment
        case .respaViolation:
            return .respaNoticeOfErrorViolation
        case .tilaViolation:
            return .tilaAPRViolation
        default:
            return .incorrectBalance
        }
    }

    /// Map error severity to audit result severity
    private func mapErrorSeverityToAuditSeverity(_ severity: MortgageErrorDetectionService.ErrorSeverity) -> AuditResult.Severity {
        switch severity {
        case .critical:
            return .critical
        case .high:
    ...
    /// Map detection tier to audit result detection method
    private func mapDetectionTierToMethod(_ tier: MortgageErrorDetectionService.DetectionTier) -> AuditResult.DetectionMethod {
        switch tier {
        case .ruleBased:
...
/// Comprehensive result structure for document analysis with tiered error detection
public struct DocumentAnalysisResult {
    let document: MortgageDocument
    let analysisResult: MortgageErrorDetectionService.AnalysisResult
    let processingMetrics: ProcessingMetrics

    /// Summary of analysis effectiveness
    public var effectivenessSummary: EffectivenessSummary {
        let ruleBasedDetectionRate = Double(analysisResult.analysisMetrics.errorsDetectedByRules) /
                                   max(Double(analysisResult.detectedErrors.count), 1.0)

        let costEfficiency = analysisResult.detectedErrors.isEmpty ? 0.0 :
                           Double(analysisResult.detectedErrors.count) / analysisResult.costBreakdown.totalCost

        return EffectivenessSummary(
            totalErrorsFound: analysisResult.detectedErrors.count,
            criticalErrorsFound: analysisResult.detectedErrors.filter { $0.severity == .critical }.count,
            ruleBasedDetectionRate: ruleBasedDetectionRate,
            aiAnalysisUtilized: analysisResult.analysisMetrics.aiAnalysisTime != nil,
            costEfficiency: costEfficiency,
            overallConfidence: analysisResult.analysisMetrics.overallConfidence,
            recommendationCount: analysisResult.recommendedActions.count
        )
    }

    public struct EffectivenessSummary {
        let totalErrorsFound: Int
        let criticalErrorsFound: Int
        let ruleBasedDetectionRate: Double // Percentage of errors caught by rules vs AI
        let aiAnalysisUtilized: Bool
        let costEfficiency: Double // Errors found per dollar spent
        let overallConfidence: Double
        let recommendationCount: Int

        public var ruleBasedEffectiveness: String {
            if ruleBasedDetectionRate >= 0.8 {
                return "Excellent - Rules caught \(Int(ruleBasedDetectionRate * 100))% of errors"
            } else if ruleBasedDetectionRate >= 0.6 {
                return "Good - Rules caught \(Int(ruleBasedDetectionRate * 100))% of errors"
            } else {
                return "Fair - Rules caught \(Int(ruleBasedDetectionRate * 100))% of errors, AI analysis valuable"
            }
        }

        public var costEffectivenessRating: String {
            if costEfficiency >= 10.0 {
                return "Excellent cost efficiency"
            } else if costEfficiency >= 5.0 {
                return "Good cost efficiency"
            } else if costEfficiency >= 2.0 {
                return "Fair cost efficiency"
            } else {
                return "Consider reviewing cost thresholds"
            }
        }
    }
}
...
public struct ProcessingMetrics {
    ...
    let errorDetectionCoverage: Double

    public var qualityAssessment: String {
        let avgQuality = (ocrConfidence + extractionConfidence + errorDetectionCoverage) / 3.0

        if avgQuality >= 0.9 {
            return "High quality processing"
        } else if avgQuality >= 0.7 {
            return "Good quality processing"
        } else if avgQuality >= 0.5 {
            return "Acceptable quality processing"
        } else {
            return "Processing quality concerns - manual review recommended"
        }
    }

    public var performanceRating: String {
        if processingTime <= 30.0 {
            return "Fast processing"
        } else if processingTime <= 60.0 {
            return "Normal processing speed"
        } else {
            return "Slow processing - consider optimization"
        }
    }
}

// MARK: - Progress Tracking Enhancement

extension DocumentProcessor.ProcessingStep {
    static let initialization = DocumentProcessor.ProcessingStep(rawValue: "initialization") ?? .preprocessing
    static let analysis = DocumentProcessor.ProcessingStep(rawValue: "analysis") ?? .dataExtraction
    static let resultValidation = DocumentProcessor.ProcessingStep(rawValue: "result_validation") ?? .completion
}
