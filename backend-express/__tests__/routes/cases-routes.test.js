 ...
 *   POST /v1/cases, GET /v1/cases, GET /v1/cases/:caseId,
 *   PUT /v1/cases/:caseId, DELETE /v1/cases/:caseId,
 *   POST /v1/cases/:caseId/documents, DELETE /v1/cases/:caseId/documents/:documentId
 *
...
describe('POST /v1/cases', () => {
  it('returns 201 with created case', async () => {
    ...
    expect(res.body.case_name).toBe('Test Case');
    expect(mockCaseFileService.createCase).toHaveBeenCalledWith(
  ...
  it('returns 400 when caseName is missing', async () => {
    const res = await request(app)
      .post('/v1/cases')
      .set('Authorization', 'Bearer valid-token')
      .send({});
  ...
  it('returns 400 when caseName exceeds 200 chars', async () => {
    const res = await request(app)
      .post('/v1/cases')
      .set('Authorization', 'Bearer valid-token')
      .send({ caseName: 'x'.repeat(201) });
  ...
  it('returns 500 on service error', async () => {
    mockCaseFileService.createCase.mockRejectedValue(new Error('DB error'));
...
describe('GET /v1/cases', () => {
  it('returns 200 with array of cases', async () => {
    const cases = [
      { id: 'case-1', case_name: 'Case A' },
      { id: 'case-2', case_name: 'Case B' }
    ];
  ...
  it('passes status filter to service', async () => {
    mockCaseFileService.getCasesByUser.mockResolvedValue([]);
    ...
    expect(mockCaseFileService.getCasesByUser).toHaveBeenCalledWith(
      expect.objectContaining({
        userId: 'mock-user-id-12345',
        status: 'open'
      })
  ...
  it('returns 500 on service error', async () => {
    mockCaseFileService.getCasesByUser.mockRejectedValue(new Error('DB unavailable'));
...
describe('GET /v1/cases/:caseId', () => {
  it('returns 200 with case and documents', async () => {
    const caseData = {
      id: 'case-1',
      case_name: 'Test Case',
      documents: [{ document_id: 'doc-1' }]
    };
    mockCaseFileService.getCase.mockResolvedValue(caseData);
  ...
  it('passes userId from JWT to service', async () => {
    mockCaseFileService.getCase.mockResolvedValue({ id: 'case-1' });
  ...
  it('returns 500 on service error', async () => {
    mockCaseFileService.getCase.mockRejectedValue(new Error('DB timeout'));
...
describe('PUT /v1/cases/:caseId', () => {
  it('returns 200 with updated case', async () => {
    ...
    mockCaseFileService.updateCase.mockResolvedValue(updatedCase);

    const res = await request(app)
      .put('/v1/cases/case-1')
      .set('Authorization', 'Bearer valid-token')
      .send({ caseName: 'Updated Case', status: 'in_review' });

    expect(res.status).toBe(200);
    expect(res.body.case_name).toBe('Updated Case');
  });

  it('returns 404 when case not found', async () => {
    mockCaseFileService.updateCase.mockResolvedValue(null);

    const res = await request(app)
      .put('/v1/cases/nonexistent')
      .set('Authorization', 'Bearer valid-token')
      .send({ caseName: 'Updated' });
  ...
  it('passes updates and userId to service', async () => {
    mockCaseFileService.updateCase.mockResolvedValue({ id: 'case-1' });

    await request(app)
      .put('/v1/cases/case-1')
      .set('Authorization', 'Bearer valid-token')
      .send({ caseName: 'New Name', notes: 'Updated notes' });

    expect(mockCaseFileService.updateCase).toHaveBeenCalledWith({
      caseId: 'case-1',
      userId: 'mock-user-id-12345',
      updates: { caseName: 'New Name', notes: 'Updated notes' }
    });
  });
  ...
  it('returns 500 on service error', async () => {
    mockCaseFileService.updateCase.mockRejectedValue(new Error('DB error'));
...
describe('DELETE /v1/cases/:caseId', () => {
  it('returns 200 with success', async () => {
    mockCaseFileService.deleteCase.mockResolvedValue({ success: true });

    const res = await request(app)
      .delete('/v1/cases/case-1')
      .set('Authorization', 'Bearer valid-token');

    expect(res.status).toBe(200);
    expect(res.body.success).toBe(true);
    expect(mockCaseFileService.deleteCase).toHaveBeenCalledWith({
      caseId: 'case-1',
      userId: 'mock-user-id-12345'
    });
  });
  ...
  it('returns 500 on service error', async () => {
    mockCaseFileService.deleteCase.mockRejectedValue(new Error('Case not found'));
...
describe('POST /v1/cases/:caseId/documents', () => {
  it('returns 200 with association result', async () => {
    const association = { document_id: 'doc-1', case_id: 'case-1' };
    mockCaseFileService.addDocumentToCase.mockResolvedValue(association);
    ...
    expect(res.body.case_id).toBe('case-1');
    expect(mockCaseFileService.addDocumentToCase).toHaveBeenCalledWith({
      caseId: 'case-1',
      documentId: 'doc-1',
      userId: 'mock-user-id-12345'
    });
  ...
  it('returns 500 on service error', async () => {
    mockCaseFileService.addDocumentToCase.mockRejectedValue(new Error('Case not found'));
...
describe('DELETE /v1/cases/:caseId/documents/:documentId', () => {
  it('returns 200 with removal result', async () => {
    const removal = { document_id: 'doc-1', case_id: null };
    mockCaseFileService.removeDocumentFromCase.mockResolvedValue(removal);

    const res = await request(app)
      .delete('/v1/cases/case-1/documents/doc-1')
      .set('Authorization', 'Bearer valid-token');

    expect(res.status).toBe(200);
    expect(res.body.document_id).toBe('doc-1');
    expect(res.body.case_id).toBeNull();
    expect(mockCaseFileService.removeDocumentFromCase).toHaveBeenCalledWith({
      documentId: 'doc-1',
      userId: 'mock-user-id-12345'
    });
  });
  ...
  it('returns 500 on service error', async () => {
    mockCaseFileService.removeDocumentFromCase.mockRejectedValue(new Error('Document not found'));
